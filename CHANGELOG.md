...
- Fixed an issue where fullscreen browser could remain undestroyed when it had been created but never shown.

## [2.13.0] - 2026-07-23

- Added `KlarnaPaymentPresentationContent` support to `KlarnaPaymentButton` via new constructor or the `setContent` method.
- Extended authentication browser support from `FragmentActivity` to hosts based on `ComponentActivity`.

## [2.12.1] - 2026-07-14

- Improved `KlarnaPaymentView`'s error handling.
- Anonymized URLs in analytic events to avoid capturing sensitive information, if any.
- Restricted authentication and sandbox browser features based on the host activity type.
- Fixed an issue that could occur when logging analytic events during SDK initialization.
...
- :warning: **This version has some functionality issues**: Please use a newer version of the SDK.
